...
app.MapPost("/notifications", async (Notification notification) =>
{
    ...
    await channel.QueueDeclareAsync(queue: "reap", durable: true, exclusive: false, autoDelete: false,
        arguments: null);

    var message = JsonSerializer.Serialize(notification);
    var body = Encoding.UTF8.GetBytes(message);

    await channel.BasicPublishAsync(exchange: string.Empty, routingKey: "reap", body: body);
